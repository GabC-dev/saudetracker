...
const METRICAS_CONFIG = {
  Peso:     { bg: '#e8f8ef', label: 'Peso',     unit: 'kg' },
  Pressao:  { bg: '#fef0f0', label: 'Pressão',  unit: 'mmHg' },
  Sono:     { bg: '#eef2ff', label: 'Sono',     unit: 'h' },
  Humor:    { bg: '#fffbeb', label: 'Humor',    unit: '/10' },
  Glicemia: { bg: '#f0f9ff', label: 'Glicemia', unit: 'mg/dL' },
}
...
export default function RegistrarMetrica() {
  ...
  return (
    <div className="page" style={{ maxWidth: 540 }}>
      ...
      {/* Formulário de métrica — só aparece após selecionar usuário */}
      {usuarioSelecionado && (
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: '1.25rem' }}>
            {Object.entries(METRICAS_CONFIG).map(([t, cfg]) => (
              <button
                key={t}
                onClick={() => { setTipo(t); setValor1(''); setValor2(''); setErro(null) }}
                style={{
            ...
            {tipo === 'Humor' && valor1 && (
              <div style={{
                ...
                <span style={{ fontSize: 22 }}>
                  {valor1 <= 3 ? '😞' : valor1 <= 5 ? '😐' : valor1 <= 7 ? '🙂' : '😄'}
                </span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--green-dark)' }}>
                  {valor1 <= 3 ? 'Humor muito baixo' :
                   valor1 <= 5 ? 'Humor baixo' :
                   valor1 <= 7 ? 'Humor moderado' : 'Humor bom'}
                </span>
              </div>
            )}
